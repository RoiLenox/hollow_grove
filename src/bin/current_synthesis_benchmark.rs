...
use hollow_grove::hueman_support::{
    ...
    build_hueman_stonebend_roles_from_artifacts, build_vertical_integration_stack_from_artifacts,
};
use hollow_grove::{
...
struct BenchmarkPaths {
    ...
    hueman_flynt_constitution: PathBuf,
...
impl BenchmarkPaths {
    fn new(root: &Path) -> Self {
        Self {
            ...
            hueman_flynt_constitution: root.join(HUEMAN_FLYNT_CONSTITUTION_ARTIFACT_PATH),
...
fn run_pipeline_sample_with_paths(
    ...
    let (hueman_flynt_constitution, stage) = measure_artifact_stage(
        &mut session,
        &paths.hueman_flynt_constitution,
        sample_index,
        "hueman_flynt_constitution",
        "hueman",
        || {
            Ok(build_hueman_flynt_constitution_from_artifacts(
                &hueman_start_choices,
                &hueman_fourway,
            ))
        },
